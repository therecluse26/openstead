...
import { ServiceLogFormData } from '@/types/Service'
...
const ServiceLogForm: React.FC<ServiceLogFormProps> = ({
    inline = false,
    serviceable_type,
    onComplete = () => {},
    onClose = () => {},
    ...
    const getFormErrorMessage = (name: keyof ServiceLogFormData) => {
        return (
            errors[name] && (
    ...
    const onSubmit: SubmitHandler<ServiceLogFormData> = async data => {
        await csrf()
        const serviceLogData = {
            ...data,
            serviceable_type: serviceable_type,
        }
        ServiceLogService.addServiceLog(id!, serviceLogData)
            .then(() => {
                if (inline) {
                    onComplete()
                    return
