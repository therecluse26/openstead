...
interface ProjectItemFormData {
    ...
    description: string
    project_id: string
    project_item_status_id: string | null
    due_date: Date | null
...
const ProjectItemForm: React.FC<ProjectItemFormProps> = ({
    ...
    const user = useAuthorizationStore(state => state.user) as User | null
    const { showToast } = useToast()
    const defaultValues: ProjectItemFormData = {
        title: '',
        description: '',
    ...
    const {
        ...
        handleSubmit,
        setValue,
    } = useForm<ProjectItemFormData>({ defaultValues })
    ...
    const getFormErrorMessage = (name: keyof ProjectItemFormData) => {
        return (
            errors[name] && (
    ...
    const onSubmit: SubmitHandler<ProjectItemFormData> = async data => {
        await csrf()
